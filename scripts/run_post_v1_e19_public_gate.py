"""Run the registered public-only E19 coarse orientation-preserving map gate."""
...
EXPERIMENT_ID = "postv1_e19_coarse_orientation_preserving_r01"
REPORT_SCHEMA = "post_v1_e19_public_coarse_orientation_gate.v1"
SEED = 20260831
REPETITIONS = 2
NODES_PER_AXIS = 8
CONTROL_COUNT = 512
...
MAXIMUM_CERTIFICATE_SECONDS = 60.0
...
def _run_repetition(
    ...
    *,
    ...
    if (
        ...
        raise AssertionError("registered E19 control profile changed")
    ...
    for name, target in p3._trajectory_proposals(carrier_vertices, carrier_faces, fixture):
        ...
        try:
            step = fit_and_certify_coarse_orientation_step(
                lattice,
                carrier_vertices,
                carrier_faces,
                carrier_surface,
                ...
                timeout_seconds=MAXIMUM_CERTIFICATE_SECONDS,
            )
        ...
        e17._write_npz_immutable(
            ...
            raw_controls=step.raw_controls,
            accepted_controls=step.accepted_controls,
            accepted_alpha=np.asarray(step.accepted_alpha, dtype=np.float64),
...
def run_public_gate(artifact_root: Path) -> dict[str, Any]:
    ...
    controls = run_coarse_orientation_controls()
    blockers: list[str] = []
    if not git["implementation_tree_clean"]:
        blockers.append("implementation_tree_not_clean")
    if controls["status"] != "pass":
        blockers.append("coarse_orientation_controls")
    repetitions: list[dict[str, Any]] = []
    if not blockers:
        constructor, auditor = e17._build_tools()
        with tempfile.TemporaryDirectory(prefix="frayid-e19-public-") as directory:
            root = Path(directory)
    ...
    return {
        ...
        "limits": {
            ...
            "certificate_seconds_per_proposal": MAXIMUM_CERTIFICATE_SECONDS,
        ...
        "bindings": {
            ...
            "mechanism_source_sha256": e17._sha256(
                PROJECT_ROOT / "src/frayid/coarse_orientation_map.py"
            ),
...
def main() -> None:
    ...
    if arguments.output.exists():
        raise FileExistsError(f"immutable E19 report exists: {arguments.output}")
    if artifact_root.exists():
        raise FileExistsError(f"immutable E19 artifact directory exists: {artifact_root}")
    artifact_root.mkdir(parents=True, exist_ok=False)
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="frayid-e19-supervisor-") as directory:
