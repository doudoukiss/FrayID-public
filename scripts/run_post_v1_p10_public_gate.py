"""Run the preregistered public P10 isolated-upstream arbitration gate."""
...
from frayid.isolated_upstream_arbitration import (
    IsolatedUpstreamArbitration,
    arbitrate_isolated_upstream,
)

CORRECTNESS_ID = "postv1_p10_isolated_upstream_arbitration_r01"
REPORT_SCHEMA = "post_v1_p10_public_isolated_upstream_arbitration_gate.v1"
REGISTERED_REVISION = "d6b9794"
MAXIMUM_ARBITRATION_SECONDS = 300.0
MAXIMUM_BATCHED_SECONDS = 5.0
MAXIMUM_ORACLE_SECONDS = 120.0
MAX_SECONDS = 30 * 60


def _repeat_arbitration(
    collision_mesh: Any,
    ...
    *,
    ...
    first = arbitrate_isolated_upstream(
        collision_mesh, vertices_t0, vertices_t1, dhat=dhat, dmin=dmin
    )
    second = arbitrate_isolated_upstream(
        collision_mesh, vertices_t0, vertices_t1, dhat=dhat, dmin=dmin
    )
    deterministic = bool(
        ...
        and np.array_equal(first.selected_contributions, second.selected_contributions)
    ...
    arbitration_seconds = first.arbitration_seconds + second.arbitration_seconds
    if not deterministic:
        blockers.append("full_arbitration_nondeterminism")
    if arbitration_seconds > MAXIMUM_ARBITRATION_SECONDS:
        blockers.append("isolated_arbitration_time")
    return dataclasses.replace(
        first,
        status="pass" if not blockers else "fail",
        coefficient_seconds=max(first.coefficient_seconds, second.coefficient_seconds),
        scalar_seconds=max(first.scalar_seconds, second.scalar_seconds),
...
def run_public_gate() -> dict[str, Any]:
    ...
    try:
        p9_module.certify_candidate_contributions = _repeat_arbitration
        p9.MAXIMUM_BATCHED_SECONDS = MAXIMUM_BATCHED_SECONDS
        p9.MAXIMUM_ORACLE_SECONDS = MAXIMUM_ORACLE_SECONDS
        with tempfile.TemporaryDirectory(prefix="frayid-p10-public-") as directory:
    ...
    return {
        ...
        "gate": "public_isolated_upstream_arbitration_mesh_certificate",
        "status": "pass" if not blockers else "fail",
...
def _failure_report(failure_class: str, started: float, exitcode: int | None) -> dict[str, Any]:
    return {
        "schema_version": "post_v1_p10_public_failure_report.v1",
        "correctness_id": CORRECTNESS_ID,
        "status": "fail",
...
def main() -> None:
    ...
    if arguments.output.exists():
        raise FileExistsError(f"immutable P10 report exists: {arguments.output}")
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="frayid-p10-supervisor-") as directory:
