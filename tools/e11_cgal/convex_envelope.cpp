...
std::vector<Point> deterministic_general_position(const std::vector<Point>& input) {
  double lower[3] = {CGAL::to_double(input.front().x()), CGAL::to_double(input.front().y()),
                     CGAL::to_double(input.front().z())};
  ...
  const double scale = std::max({upper[0] - lower[0], upper[1] - lower[1],
                                 upper[2] - lower[2]});
...
int main(int argc, char** argv) {
  try {
    if (argc != 3) {
      std::cerr << "usage: frayid_e11_convex_envelope INPUT.e6mesh OUTPUT.e10mesh\n";
      return 2;
    }
    const std::vector<Point> input = read_input_vertices(argv[1]);
    const std::vector<Point> hull_input = deterministic_general_position(input);
    ...
    Kernel::FT center_x(0), center_y(0), center_z(0);
    for (const Point& point : input) {
      center_x += point.x();
      center_y += point.y();
      center_z += point.z();
    }
    const Kernel::FT count(static_cast<long>(input.size()));
    const Point center(center_x / count, center_y / count, center_z / count);
    const Kernel::FT expansion = Kernel::FT(101) / Kernel::FT(100);
    for (const auto vertex : envelope.vertices()) {
      const Point& point = envelope.point(vertex);
      envelope.point(vertex) = center + expansion * (point - center);
    }
    ...
    for (const auto face : envelope.faces()) {
      ...
      for (const auto vertex : CGAL::vertices_around_face(envelope.halfedge(face), envelope)) {
        if (corner >= 3) fail("convex envelope contains a non-triangle face");
    ...
    std::cerr << "E11_CONVEX_ENVELOPE_ERROR: " << error.what() << "\n";
    return 1;
  }
}
